...
class SrcEngCollProperties(bpy.types.PropertyGroup):
    ...
    Bisect_Mode: bpy.props.EnumProperty(
        items= [
                ...
                ("xyz", "Hybrid", "A blend of the above two options, resulting in cuts along all axes"),
                ],
        name="Mode",
    ...
    def execute(self, context):
        ...
        if len(objs) >= 1:
            ...
            for obj in objs:
                ...
                # Check if the object is made up entirely of nonmanifold geometry or has extremely low face count, and if so, skip it.
                bpy.ops.object.mode_set(mode="EDIT")
                bpy.ops.mesh.select_mode(use_extend=False, use_expand=False, type='VERT')
                bpy.ops.mesh.reveal()
                bpy.ops.mesh.select_all(action='DESELECT')
                ...
                nonmanifold_count = len([vert for vert in obj.data.vertices if vert.select == True])
                print(f"Nonmanifold verts in {obj.name} = {str(nonmanifold_count)}")
                vertex_count = len(obj.data.vertices)
                print(f"Total verts in {obj.name} = {str(vertex_count)}")
                
                if nonmanifold_count == vertex_count or vertex_count < 4:
                    print(f"{obj.name} contains invalid geometry. Skipping...")
                    continue
            ...
            if total_hull_count > 0:
                display_msg_box(
                    "Generated collision mesh, with total hull count of " + str(total_hull_count) + ".", "Info", "INFO")
            else:
                display_msg_box(
                    "No collision meshes were generated. Select a different mesh and try again.", "Info", "INFO")
    ...
    def execute(self, context):
        ...
        def solidify_and_bool(obj_phys, obj_bbox, mode="xy"):
            ...
            for split_part in split_parts:
                ...
                bool_dimensions = get_modified_dimensions(split_part)
        ...
        if len(objs) >= 1:
            ...
            for obj in objs:
                ...
                if slice_mode == 'xy':
                    ...
                    pre_clean(obj_phys, obj_bbox)
                    cull_boundaries(obj_bbox, mode="xy")
                    solidify_and_bool(obj_phys, obj_bbox, mode="xy")

                    bpy.context.view_layer.objects.active = obj
                    obj_phys.select_set(False)
                    obj.select_set(True)
                    
                    obj_phys2, obj_bbox2 = gen_bisect_setup(obj)
                    auto_bisect(obj_bbox2, cuts=cuts, mode="z")
                    cull_edges(obj_bbox2, threshold=0.1, mode="z")
                    fill_bbox(obj_phys2, obj_bbox2)
                    pre_clean(obj_phys2, obj_bbox2)
                    cull_boundaries(obj_bbox2, mode="z")
                    solidify_and_bool(obj_phys2, obj_bbox2, mode="z")

                    hybrid_boolean = obj_phys.modifiers.new(name="HybridBool", type='BOOLEAN')
                    hybrid_boolean.object = obj_phys2
                    hybrid_boolean.operation = 'INTERSECT'
                    hybrid_boolean.solver = 'EXACT'
                    
                    with bpy.context.temp_override(object=obj_phys):
                        bpy.ops.object.modifier_apply(modifier="HybridBool")
                    bpy.data.objects.remove(obj_phys2)
...
# Shrinkwrap to Visuals operator

class Cleanup_Shrinkwrap(bpy.types.Operator):
    """Tightens the shape of the selected collision mesh(es) by shrinkwrapping to the original visual (source) mesh, while maintaining convexity. Has no effect if the original visual mesh can not be found"""
    bl_idname = "object.src_eng_cleanup_shrinkwrap"
    bl_label = "Shrinkwrap to Visuals"
    bl_options = {'REGISTER'}
    ...
    def execute(self, context):
        ...
        if len(objs) >= 1:
            ...
            for obj in objs:
                if "_phys" not in obj.name:
                    continue
                
                original_name = obj.name.split("_phys")[0]
                if original_name not in bpy.context.scene.objects.keys():
                    missing += 1
                    continue
                ...
                shrinkwrap_modifier = obj.modifiers.new(name="ShrinkwrapPhys",type="SHRINKWRAP")
                shrinkwrap_modifier.target = bpy.data.objects[original_name]
            ...
            confirmation_msg = "Shrinkwrapped " + str(count) + " collision mesh(es)."
            if missing > 0:
                confirmation_msg = confirmation_msg + "\n" + str(missing) + " visual mesh(es) couldn't be found."
            display_msg_box(confirmation_msg, "Info", "INFO")
